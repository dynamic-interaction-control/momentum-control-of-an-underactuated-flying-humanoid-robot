function [basePose,jointAngles,baseVelocity,jointsVelocity] = stateDemux(state,config)
%#codegen
    % state  
    ...
    basePose        = zeros(4);
    if config.demux.baseOrientationType == 0
       % I_R_b       = quaternion2dcm(Q_b);
        I_R_b       =  WBM.utilities.quat2rotm(Q_b);
        basePose    = [I_R_b,I_p_b;zeros(1,3),1];
     elseif config.demux.baseOrientationType == 1
